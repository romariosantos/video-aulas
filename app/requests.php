	...
	switch ($GET) {
		case 'addnot':
			...
			if(!empty($id_user) and is_numeric($id_user)){
				...
				if($insert){
					echo 'Sucesso!';
				}else { echo 'Não foi possível inserir notificação'; }
			}
		...
		case 'getnots' :
			...
			while($rows = $get->fetch(PDO::FETCH_OBJ)){
				$li .= '<li class="n" id="'.$rows->idn.'">';
					...
					$li .= '<div class="contn">';
						$li .= '<span>'.$rows->nome.'</span> curtiu seu status em <span>'.date('d/m/Y H:i', strtotime($rows->data)).'</span> curte ai galera...';
					$li .= '</div>'; 
 					
 					$vis   = ($rows->status == 0) ? 'vis' : 'vis2';
 					$title = ($rows->status == 0) ? 'Não visualizado' : 'Visualizado';
